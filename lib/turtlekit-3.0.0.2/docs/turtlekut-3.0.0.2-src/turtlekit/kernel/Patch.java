...
public class Patch {
	...
	public Patch(int i, int j, TKEnvironment abstractEnvironment) {
		this(i, j);
		this.env = abstractEnvironment;
		neighbors.add(this);// for the include
	}
	...
	public List<Patch> getNeighbors(int inRadius, boolean includeThisPatch) {
		final int height = env.getHeight();
		final int width = env.getWidth();
  ...
  @param value  mark itself, can be any java object*/
	...
	public void dropPheromone(String name, float quantity, Float... parameters) {
		env.getPheromone(name).incValue(x, y, quantity);
	}
