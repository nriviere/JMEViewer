...
public class CudaPheromoneV3 extends turtlekit.pheromone.Pheromone implements CudaObject{
...
//	private Runnable diffusionAndEvaporation;
	...
	public CudaPheromoneV3(String name, int width, int height, final int evapPercentage,
...
//		arr[0]=1;
//		System.err.println(values.capacity());
//		System.err.println(arr.length);
		values.rewind();
		try {
			values.put(arr,0,arr.length);
		} catch (Exception e) {
...
//		final int j = getWidth()*getHeight();
		values.rewind();
		int i = 0;
		try {
			while (true) {
